...
(function(window, msetools, undefined) {
  ...
  WebMValidator.prototype.abort = function(data) {
  };
  ...
  /**
   * Checks to see if a WebM ID is a list.
   * @param {number} id A WebM ID.
  ...
  function parseWebMId_(buf) {
    ...
    /** @type {number} */ var id = buf[0];
    var allOnes = (id & allOnesMask) == allOnesMask;
   ...
   * @param {?Uint8Array} buf The buffer to parse.
   * @return {{status: msetools.ParserStatus, bytesUsed: number, id: number,
   * size: number}}
   */
  WebMValidator.prototype.parseElementHeader = function(buf) {
    ...
    if (result.status != msetools.ParserStatus.OK)
      return {status: result.status, bytesUsed: 0, id: 0, size: 0 };
    /** @type {{status: msetools.ParserStatus, bytesUsed: number,
        id: number}} */(result);
    var id = result.id;
